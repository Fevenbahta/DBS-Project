...
public class RtgRepositoryAPI : IRtgRepositoryAPI
{
    private readonly HttpClient _httpClient;

    public RtgRepositoryAPI(HttpClient httpClient)
    {
    ...
    public async Task<FinInsInsResponseDTO> CreateRtgsTransfer(RtgTransactionRequest request)
    {
    ...
    private async Task<FinInsInsResponseDTO> CheckTransactionStatus(string referenceNumber, string token)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode && responseObject.CheckStatusResponse.Successful == true)
            {
                ...
                {
                    success = true,
                    message = "RTGS Transaction successful",
                    FinInstransactionId = referenceNumber,
                    ConversationID = string.Empty
                };
            }
            else
            {
                ...
                {
                    success = false,
                    message = "RTGS Transaction failed",
                    FinInstransactionId = string.Empty,
                    ConversationID = string.Empty
                };
            }
